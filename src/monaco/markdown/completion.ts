...
export class MdCompletionItemProvider implements languages.CompletionItemProvider {
    triggerCharacters = ['(', '\\', '/', '[', '#']
    ...
    constructor() {
        ...
        let c1 = latex._c1.map(cmd => {
            let item = newCompletionItem('\\' + cmd, 1); // languages.CompletionItemKind.Function
            item.insertText = cmd;
            return item;
        });
        // \cmd{$1}
        let c2 = latex._c2.map(cmd => {
            ...
            item.insertTextRules = 4; // languages.CompletionItemInsertTextRule.InsertAsSnippet
        ...
        let envSnippet = newCompletionItem('\\begin', 27); // languages.CompletionItemKind.Snippet
        ...
        envSnippet.insertTextRules = 4; // languages.CompletionItemInsertTextRule.InsertAsSnippet

        this.mathCompletions = [...c1, ...c2, ...c3, envSnippet];
        // Sort
        this.mathCompletions.forEach(item => {
            item.sortText = (item.label as string).replace(/[a-zA-Z]/g, c => {
                if (/[a-z]/.test(c)) {
                    return `0${c}`;
                } else {
        ...
        if (
            ...
            return new Promise((res, _) => {
                ...
                let refLabels = lines.reduce((prev, curr) => {
                    ...
                    if ((match = /^\[([^\]]*?)\]: (\S*)( .*)?/.exec(curr)) !== null) {
                        const ref = match[1];
                        let item = newCompletionItem(ref, 21); // languages.CompletionItemKind.Reference
            ...
            return new Promise((res, _) => {
                ...
                const headingCompletions: languages.CompletionItem[] = toc.reduce((prev: languages.CompletionItem[], curr: any) => {
                    let item = newCompletionItem('#' + slugify(curr.text), 21); // languages.CompletionItemKind.Reference
